...
export class AddRoomTypeComponent implements OnInit {
  ...
  onSubmitData() {
    ...
    this.spinner.show();
    this.roomTypeApiServiceService.save(this.form.value).subscribe(
      res => {
        if (this.type === 'add') {
  ...
  getUserDetail(id) {
    this.roomTypeApiServiceService.getInfo(id).subscribe(
      res => {
  ...
  private buildForm() {
    if (this.type === 'add') {
      this.title = 'Thêm mới loại phòng';
    } else if (this.type === 'update') {
      this.title = 'Sửa loại phòng';
    } else this.title = 'Xem chi tiết loại phòng';
